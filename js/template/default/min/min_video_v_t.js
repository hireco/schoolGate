...
$(document).ready(function() {
	...
	$.post(get_url(json_str.base_url+'ajax/cms_ajax/cms_relate_list/video'),{id:$('#entry_id').val()},function(data){
		$('.list_body').removeClass('list_body_ajax');
		if(data) $('.list_body').html(data);
		else $('.list_body').html('<li>暂无相关内容</li>');
	}); 
	
	show_comment_form($('#entry_id').val(),'video','0');
	show_comment_list($('#entry_id').val(),'video');
	
	$('#show_comment_form').click(function(){
		if($('.comment_form').length==0) top_message('当前内容禁止评论！');
		else if(!$('.comment_form').html()) show_comment_form($('#entry_id').val(),'video','0');
	});
	
	if($('.video_html embed').attr('height')!='40') 
	$('.video_html embed').attr('width','600');
