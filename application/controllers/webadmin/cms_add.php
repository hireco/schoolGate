...
class Cms_add extends CI_Controller {
	...
	private function viewer($model_id, $jsfile) {
		$data['model_id']=$model_id;
		$data['class_select_option']=$this->cms_model->class_select_option();
		
		$data['workplace_view']=$this->load->view('admin/cms_add',$data,TRUE);
		...
		if($jsfile) array_push($jsmin, $jsfile);		
		$data['jsmin']=$this->minify->js_mini($jsmin);
	...
	public function article() {
		$model_id=$this->myconfig->cms_model_id(__FUNCTION__);
		$jsfile='';
		$this->viewer($model_id, $jsfile);
	}
	...
	public function submit_download(){
		...
		if(!$this->input->post('cms_submit')) return $this->echo_infor('非法操作');		
		...
		if($this->db->affected_rows()) {
			...
			$this->db->insert('cms_download',$data);
			$content_id=$this->db->insert_id();
			$this->db->where('index_id',$index_id);
			$this->db->update('cms_index',array('content_id' => $content_id));
		}
			
		if($this->db->affected_rows())  return $this->echo_infor('下载发布成功','1','',$index_id);
		else return $this->echo_infor('下载发布失败，请重试');
	...
	public function submit_video(){
		...
		if($this->db->affected_rows()) {
			...
			$this->db->insert('cms_video',$data);
			$content_id=$this->db->insert_id();
			$this->db->where('index_id',$index_id);
			$this->db->update('cms_index',array('content_id' => $content_id));
		}
			
		if($this->db->affected_rows())  return $this->echo_infor('视频发布成功','1','',$index_id);
		else return $this->echo_infor('视频发布失败，请重试');
	...
	public function submit_photo(){
		...
		if($this->db->affected_rows()) {
			...
			$this->db->insert('cms_photo',$data);
			$content_id=$this->db->insert_id();
			$this->db->where('index_id',$index_id);
			$this->db->update('cms_index',array('content_id' => $content_id));
		}
			
		if($this->db->affected_rows())  return $this->echo_infor('图集发布成功','1','',$index_id);
		else return $this->echo_infor('图集发布失败，请重试');
	...
	public function submit_article(){
		...
		if($this->db->affected_rows()) {
			$index_id=$this->db->insert_id();
			$this->db->insert('cms_article',array('index_id'=> $index_id, 'article_body' => $article_body));
			$content_id=$this->db->insert_id();
			$this->db->where('index_id',$index_id);
			$this->db->update('cms_index',array('content_id' => $content_id));
		}
			
		if($this->db->affected_rows())  return $this->echo_infor('文章发布成功','1','',$index_id);
		else return $this->echo_infor('文章发布失败，请重试');
