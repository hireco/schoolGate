...
<div class="page_div" id="list_right">   
   ...
   <?php $cur_year=date('Y',time()); 
         ...
         for($i=$cur_year; $i>$cur_year-$year_back; $i--) {
           ...
           echo '"><a href="'.site_url('publication/'.$i).'" ';            
           echo '>'.$i.'</a></span>';
         }
   ?>
   </div>
...
<div class="page_div" id="list_left">
  ...
  <?php if($papers) {?>
  <div class="part_header">发表论文</div>
  <div class="part_body">
  <?php foreach ($papers as $index => $value) {
      ...
      if($pub_file[0]=='1' && $pub_file[1]) {
      	...
      	$filepath=site_url('file/'.$filepath);
 ...
 <?php }?>
  ...
  <?php if($talks) {?> 
  <div class="part_header">会议报告</div>
  <div class="part_body">
  ...
  <?php if($books) {?> 
  <div class="part_header">出版专著</div>
  <div class="part_body">
