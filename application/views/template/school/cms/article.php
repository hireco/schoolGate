<div class="body">
  <div class="page_navi">您的位置：<a href="<?php echo base_url();?>">首页</a><?php echo $this->cms_function->get_current_path($index['class_id'],'article');?> &gt; <?php echo $index['cms_title']?></div>
  <div class="page_div" id="mainbar" >
    ...
    <?php 
      if($this->myconfig->item('article_page_mode')=='server' && $content['pages']>1) {
          ...
          if($page_id!=1) echo '<a href="'.site_url('cms/view/'.$index['index_id'].'/'.($page_id-1)).'">上一页</a>';
		  for($i=1;$i<=$content['pages'];$i++) {
         	echo '<a href="'.site_url('cms/view/'.$index['index_id'].'/'.$i).'"';
         	if($i==$page_id) echo 'class="current"';
         	echo '>'.$i.'</a>';
          }
		  if($page_id!=$content['pages']) echo '<a href="'.site_url('cms/view/'.$index['index_id'].'/'.($page_id+1)).'">下一页</a>';
          echo '</div>';
      }
