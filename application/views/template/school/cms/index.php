<div class="body">
<div class="page_navi">您的位置：<a href="<?php echo base_url();?>">首页</a> > 信息中心</div>
...
<?php 
	$class_list=$this->myquery->cms_class_list(0,0);
	foreach ($class_list as $index => $value) {
		$child_entry[$value['class_id']]=$this->cms_function->cms_list_of_id($value['class_id'],8);
	}			
	
    $thumb_scale=$this->myconfig->item('thumb_keep_scale');
	$count=0;
	foreach ($class_list as $index => $value) { 
	 if(count($child_entry[$value['class_id']])>7) {
		$count++;
	...
	<div class="class_list <?php if($count%2!=0) echo 'list_even';?>">
	  ...
	  <div class="class_body" id="body_with_image">			
		<div class="icon_image_div">
		   <div class="icon_image <?php echo $thumb_scale!='1'?'rescale':'';?>">
			  <a class="thumb_wrapper" href="<?php echo site_url($table.'/'.$image_entry['cms_title_en']);?>"><img src="<?php echo $icon_image;?>" /></a>
		   </div>
		</div>
		
		<div class="icon_entry">
		   <div class="title"><a href="<?php echo site_url($table.'/'.$image_entry['cms_title_en']);?>"><?php echo $image_entry['cms_title'];?></a></div>
		   <div class="abstract"><a href="<?php echo site_url($table.'/'.$image_entry['cms_title_en']);?>"><?php echo my_limiter($image_entry['cms_description'],50);?></a></div>
		</div>
	 ...
	 <ul class="class_body">
		<?php foreach ($child_entry[$value['class_id']] as $index_i => $value_i) {
			...
			echo '<li class="list_entry"><a title="'.$value_i['cms_title'].'" href="'.site_url($table.'/'.$value_i['cms_title_en']).'">'.$value_i['cms_title'].'</a><span class="entry_time">'.date('d H:s',$value_i['post_time']).'</span></li>';
		}
		?>
	 </ul>
   </div>
