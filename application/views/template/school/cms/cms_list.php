...
<?php 
	echo '<div class="page_div" id="mainbar">';
	...
	foreach ($entry_list as $index => $value) {
		...
		echo '<div class="list_entry"><a href="'.site_url($table.'/'.$value['cms_title_en']).'">'.$value['cms_title'].'</a><span class="entry_time">'.date('m-d H:i',$value['post_time']).'</span></div>';
	}
	echo '</div>';
