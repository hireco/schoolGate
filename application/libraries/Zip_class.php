...
class zip_class {
    ...
    function ExtractAll ( $zn, $to)
    {
        ...
        if(is_array($files))
        {
            for($i=0;$i<$cn;$i++)
            {
                if($files[$i]['folder']==1)
                {
                    @mkdir($to.$files[$i]['filename'],$this->files_access_right);
                    @chmod($to.$files[$i]['filename'],$this->files_access_right);
                }
            }
        }
        $this->Extract ($zn,$to);
    ...
    function ExtractFile($header,$to,$zip)
    {
        ...
        if(!@is_dir($to))
        {
            @mkdir($to,$this->files_access_right);
            $this->total_folders ++;
        }
        ...
        $this->files_list[]=$to.$header['filename'];
        $this->total_files ++;
        return true;
    }
}
