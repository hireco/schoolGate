...
class Split_word {
    ...
    function __construct($source_charset='utf-8', $target_charset='utf-8', $load_all=TRUE, $source='')
    {
        ...
        if(file_exists($this->mainDicFile)) $this->isUnpacked = TRUE;
    ...
    */
    ...
    {
		$this->addonDicFile = $this->addonDicFile;
		$this->mainDicFile = $this->mainDicFile;
		$this->mainDicFileZip = $this->mainDicFileZip;
	...
	function InportDict( $targetfile )
     {
     	if(!ini_set('memory_limit', '128M'))
			exit('设置内存错误,请下载解压版的词库!');
     	
		$CI =& get_instance();
     	$CI->load->library('zip_class');
     	//echo $targetfile;
     	$unpackagefile = array_keys($CI->zip_class->Extract($targetfile,'data/'));
     	//exit();
